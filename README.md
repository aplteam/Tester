...
Depending on how the Test framework is used it might or might not present a GUI. Note that the GUI is a Windows-only feature although `Tester` works on all platforms; without a GUI messages of any kind are printed to the Dyalog session window.
...
The "Message" tab shows all the messages the `Tester` class provides, and prints to the Dyalog session window on non-Windows platforms:
...
That ref should point to the namespace hosting the test cases. If you are executing `#.Testers.EstablishHelpersIn` from within that namespace you can specify an empty vector as the right argument: it then defaults to `⎕IO⊃⎕RSI`.
...
All Helpers are members of the `Tester.Helpers` sub class. That means you can get a technical documentation with this statement:
